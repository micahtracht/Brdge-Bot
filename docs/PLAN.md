...
- [x] **BEN integrated**: `match.py --ben-a/--ben-b` spawns BEN clients from `vendor/ben/.venv` (Python 3.12 — BEN's DDS 3.0 extension is compiled against 3.12 only; TF 2.18.1 CPU). *(2026-08-21: 2-board smoke, BEN NS vs WBridge5 EW: BEN +5 IMPs, all from bidding (DD decomposition +5 bid / 0 play); clean exit, BEN quits on End of session. WBridge5 pinned to SAYC via committed `harness/wbridge5-sayc.ini`. Throughput: BEN ≈ 3–4 min/board/room vs WBridge5's ~1 — BEN's DDS sampling is the bottleneck as predicted; with `dds_max_threads=0` each BEN grabs all cores, so K>2 BEN tables will contend — set `dds_max_threads` per process and measure before a long run.)*
- [ ] **Baselines on record**: BEN vs WBridge5 (2,048 boards) — also our first real per-board SD measurement. Plan: a 256-board K=2 probe first (≈ 8 h; measures real SD and BEN throughput under contention), then size the full run.
